...
Check if route exists. If it does, add this new shortest route to the routes list that you created. If you’ll recall, this route returned from bfs() is a Python list that represents the path.
22.
...
Still inside the if condition, use continue to check the next starting station in the outer for loop if no possible route was found.
...
Now there is a chance that no possible route is found from any starting station. To cover this case, check that at least one route exists before calculating the shortest_route.
